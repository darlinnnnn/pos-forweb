        ...
        <div className="relative overflow-hidden mb-1.5 group/item" style={{ contain: 'layout style paint' }}>
            ...
            >
                <div className="py-4 px-4 md:px-5 flex gap-3">
                    {item.isUnsent && (
                        <div className="w-3 h-3 bg-emerald-500 rounded-full animate-pulse shadow-lg shrink-0 mt-1.5"></div>
                    )}
...
const EditItemModal: React.FC<{
    ...
    useEffect(() => {
        if (item) {
            ...
            const initialVal = item.discount ? item.discount.toString() : '';
            // Initially nominal, so format it
            setDiscountValue(initialVal.replace(/\B(?=(\d{3})+(?!\d))/g, '.'));
            setDiscountType('nominal'); // Default to nominal when opening
        }
    }, [item]);

    // Handle tab switching - clear field when switching types
    const handleTypeChange = (newType: 'nominal' | 'percent') => {
        if (newType !== discountType) {
            setDiscountValue(''); // Clear field when switching types
        }
        setDiscountType(newType);
    };

    const handleDiscountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        let val = e.target.value.replace(/[^0-9.]/g, ''); // Allow basic numeric chars

        if (discountType === 'percent') {
            // Percent logic: cap at 100
            let num = parseFloat(val);
            if (num > 100) num = 100;
            // Prevent negative
            if (num < 0) num = 0;
            setDiscountValue(isNaN(num) ? '' : num.toString());
        } else {
            // Nominal logic: auto-format thousands
    ...
    const handleSave = () => {
        let finalDiscount = 0;
        // Parse the raw number by removing dots for calculation
        ...
        finalDiscount = Math.min(finalDiscount, item.price);

        onSave(item.cartId, { notes, quantity, discount: Math.floor(finalDiscount) });
        onClose();
    };
            ...
            <div className="bg-white dark:bg-slate-900 border-t sm:border border-slate-200 dark:border-slate-700 rounded-t-3xl sm:rounded-3xl w-full max-w-sm shadow-2xl animate-in slide-in-from-bottom duration-300">
                ...
                <div className="p-6 space-y-6">
                    <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-inner">
                        ...
                        <p className="text-primary-600 dark:text-primary-400 font-black font-mono text-base mt-1">{formatCurrency(item.price)}</p>
                    </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">Item Discount</label>
                            <div className="flex bg-slate-200 dark:bg-slate-800 rounded-xl p-1 shadow-inner border border-slate-300 dark:border-slate-700">
                                <button
                                    onClick={() => handleTypeChange('nominal')}
                                    className={`px-4 py-2 rounded-lg text-sm font-black uppercase tracking-wide transition-all ${discountType === 'nominal' ? 'bg-primary-500 text-slate-900 shadow-md' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
                                >
                                    Rp
                                </button>
                                <button
                                    onClick={() => handleTypeChange('percent')}
                                    className={`px-4 py-2 rounded-lg text-sm font-black uppercase tracking-wide transition-all ${discountType === 'percent' ? 'bg-primary-500 text-slate-900 shadow-md' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
                                >
                                    %
                                </button>
                            </div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full h-16 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-5 text-slate-900 dark:text-white font-black text-3xl focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 placeholder:text-slate-300 dark:placeholder:text-slate-700 tabular-nums text-right shadow-inner"
                                placeholder="0"
                            />
                        </div>
                        {discountType === 'percent' && discountValue && (
                            <div className="text-right text-xs font-bold text-slate-500 dark:text-slate-400">
                                = {formatCurrency((item.price * (parseFloat(discountValue.replace(/\./g, '')) || 0)) / 100)} deduction
                            </div>
                        )}
                    </div>
                    <div className="space-y-3">
