...
interface OutletSelectorProps {
  outlets?: Outlet[]; // Made optional as we might fetch internal
  onSelect: (outlet: Outlet) => void;
  onBack: () => void;
  storeName: string;
  storeId?: string; // New prop for fetching
}

const OutletSelector: React.FC<OutletSelectorProps> = ({ outlets: initialOutlets = [], onSelect, onBack, storeName, storeId }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [internalOutlets, setInternalOutlets] = useState<Outlet[]>(initialOutlets);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (storeId) {
      setIsLoading(true);
      supaDataService.getOutlets(storeId).then(data => {
        setInternalOutlets(data);
      }).finally(() => setIsLoading(false));
    }
  }, [storeId]);

  const filteredOutlets = useMemo(() => {
    return internalOutlets.filter(outlet =>
      outlet.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [internalOutlets, searchQuery]);
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
          ...
          <div className="p-4 bg-slate-950/50 text-center border-t border-slate-800">
            <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">
              Showing {filteredOutlets.length} of {internalOutlets.length} available outlets
            </p>
          </div>
        </div>
