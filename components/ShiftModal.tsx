...
const ShiftModal: React.FC<ShiftModalProps> = (props) => {
  const { isOpen, onClose, shiftData, onConfirmShift, isInitial = false } = props;
  const [step, setStep] = useState<'cashier' | 'pin' | 'cash' | 'summary'>('cashier');

  // State for fetched cashiers
  const [cashierOptions, setCashierOptions] = useState<any[]>([]);
  const [loadingCashiers, setLoadingCashiers] = useState(false);

  useEffect(() => {
    if (isOpen && props.outletId) {
      setLoadingCashiers(true);
      supaDataService.getOutletUsers(props.outletId).then(users => {
        const mapped = users.map(u => ({
          id: u.id,
          name: u.name,
          pin: u.pin_code,
          requiresPin: !!u.pin_code
        }));
        setCashierOptions(mapped);
        if (mapped.length > 0) setSelectedCashier(mapped[0]);
      }).finally(() => setLoadingCashiers(false));
    }
  }, [isOpen, props.outletId]);
          ...
          {step === 'cashier' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-4" ref={cashierDropdownRef}>
                {loadingCashiers ? (
                  <div className="text-center text-slate-500 text-xs font-bold uppercase tracking-widest animate-pulse">Loading cashiers...</div>
                ) : (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">ID: {cashier.id && cashier.id.split('-')[0]}</p>
                        </div>
                      </div>
