...
const Header: React.FC<HeaderProps> = ({ shiftData, onOpenControlCenter }) => {
    ...
    <header className="h-16 shrink-0 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-4 md:px-6 z-30">
      ...
      <div className="flex items-center">
         <button 
            ...
            className="flex items-center gap-2 md:gap-3 px-2 md:px-3 py-1.5 rounded-xl hover:bg-slate-800/50 transition-all duration-200 outline-none border border-transparent hover:border-slate-700"
         >
             <div className="p-1.5 bg-slate-800 rounded-lg text-primary-500 border border-slate-700/50 shadow-inner">
                <User size={18} />
             </div>
             
             <div className="text-left hidden sm:block">
                <p className="text-sm font-bold text-white leading-tight">{isShiftOpen ? shiftData.cashierName : 'Sign In'}</p>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">Terminal 01</p>
             </div>
         </button>
      </div>
    </header>
